...
document.addEventListener('DOMContentLoaded', async () => {
    await i18n.init();
    if (document.body.dataset.page !== 'cpp-lessons') return;
    ...
    function renderExample(item) {
        ...
        if (item.type === 'alert' || item.type === 'text') return `<div class="lesson-note">${item.icon ? `<i class="${escapeHtml(item.icon)}" aria-hidden="true"></i>` : ''}<span>${escapeHtml(item.text || '')}</span></div>`;
        if (item.type === 'module-box') {
            const moduleCode = (item.content || []).map(line => `${line.code}${line.comment ? `  ${line.comment}` : ''}`).join('\n');
            return `<section class="lesson-example"><h4>${escapeHtml(label)}</h4>${renderCodeWindow({ code: moduleCode, label, filename: 'library.cpp', language: 'cpp' })}</section>`;
        }
        if (item.type === 'group') return `<div class="lesson-example-group">${(item.items || []).map(renderExample).join('')}</div>`;
        if (item.type === 'container') return `<div class="lesson-example-group">${(item.items || []).filter(child => child.type !== 'divider').map(renderExample).join('')}</div>`;
        return `<section class="lesson-example"><h4>${escapeHtml(label)}</h4>${item.code ? renderCodeWindow({ code: item.code, label, filename: 'example.cpp', language: 'cpp' }) : ''}${note ? `<p>${escapeHtml(note)}</p>` : ''}</section>`;
    }
    ...
    async function loadLessonFile(path, filename) {
        ...
        try {
            ...
            content.innerHTML = renderCodeWindow({ code: source, label: activeFileType === 'subject' ? 'ملف الموضوع' : 'ملف التحدي', filename, language: 'cpp' });
    ...
    function selectFile(type) {
        ...
        loadLessonFile(path, type === 'subject' ? `subject${activeLesson.id}.cpp` : `challenge${activeLesson.id}.cpp`);
    }
    ...
    function renderLesson(lesson) {
        ...
        lessonViewer.querySelectorAll('[data-file-type]').forEach(button => button.addEventListener('click', () => selectFile(button.dataset.fileType)));
        if (subjectPath) loadLessonFile(subjectPath, `subject${lesson.id}.cpp`);
    }

    function selectLesson(lesson, options = {}) {
        renderLesson(lesson);
        if (options.updateHash !== false) history.replaceState(null, '', `#lesson-${lesson.id}`);
        if (options.scroll) document.getElementById('lessons')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }

    lessonList.innerHTML = lessons.map(lesson => `<a href="#lesson-${escapeHtml(lesson.id)}" class="lesson-list-link" data-lesson-id="${escapeHtml(lesson.id)}"><span class="lesson-list-link__number">${escapeHtml(lesson.id)}</span><span>${escapeHtml(lesson.title)}</span></a>`).join('');
